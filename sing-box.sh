...
check_cdn() {
  ...
  for PROXY in "" "${GITHUB_PROXY[@]}"; do
    {
      CODE=$(wget --no-check-certificate -qT5 -O /dev/null --server-response "${PROXY}https://api.github.com/repos/SagerNet/sing-box/releases" 2>&1 | awk '/HTTP\//{code=$2} END{print code}')
      [ "$CODE" = '200' ] && [ ! -e "${TEMP_DIR}/cdn_proxy" ] && printf '%s' "$PROXY" > "${TEMP_DIR}/cdn_proxy"
    } &
  done

  # 等第一个返回 200 的结果，超时则回退为直连，避免无限等待卡死
